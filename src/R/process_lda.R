...
J <- function(X,data.trn)
{
   ...
   if(k > 0)
   {
      ...
      data.lda <- lda(data.trn.data,data.trn.class)
      upredict <- predict(data.lda,data.trn.data,CV=TRUE)
...
Jtest <- function(X,data.trn,data.tst)
{
   ...
   if(k > 0)
   {
      ...
      data.lda <- lda(data.trn.data,data.trn.class)
      upredict <- predict(data.lda,data.tst.data,CV=TRUE)
...
inverse <- function(partition,sizeOfZ,X)
{
   ...
   #print(features)
   #print(sizeOfZ)
   #print(as.double(realSizeOfPartitions))
